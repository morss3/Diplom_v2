...
# def get_pca_plot(X_train, y_train, target_names, method_name):
...
#     # Добавляем центры категорий и подписи
#     for i in range(len(target_names)):
#         # Находим все точки этого класса
#         points = X_pca[y_train == i]
#         if len(points) > 0:
#             # Вычисляем среднюю координату (центроид)
...
#     ax.set_title(f"PCA Projection with Cluster Centers: {method_name}", fontsize=15)
#     exp_var = pca.explained_variance_ratio_
#     ax.set_xlabel(f"PC1 ({exp_var[0]:.1%})", fontsize=12)
#     ax.set_ylabel(f"PC2 ({exp_var[1]:.1%})", fontsize=12)

#     # Создаем легенду с названиями классов вместо цветовой шкалы
#     # Находим элементы легенды, явно указывая, что нам нужно столько же меток, сколько классов
#     legend_handles = []
#     for i, name in enumerate(target_names):
#         color = cmap(i)
#         # Формируем текст: "Номер: Название"
...
#     # Выносим легенду за пределы графика, чтобы она не перекрывала точки
#     # ncol = 1 if num_classes <= 10 else 2
#     ax.legend(
#         handles=legend_handles,
#         loc="center left",
#         bbox_to_anchor=(1, 0.5),
#         title="Категории (ID: Название)",
#         fontsize=9,
#         ncol=1 # для одного столбца
#     )
...
# def get_tsne_plot(X_all, y_all, target_names, method_name):
#     # 1. Подготовка данных: t-SNE лучше работает, если сначала снизить размерность до 50
#     if X_all.shape[1] > 50:
#         pca_pre = PCA(n_components=50, random_state=42)
#         X_reduced = pca_pre.fit_transform(X_all)
#     else:
#         X_reduced = X_all

#     # 2. Обучение t-SNE
#     # perplexity 30-50 обычно оптимально для 20newsgroups
...
#     # Рисуем все точки
#     scatter = ax.scatter(X_tsne[:, 0], X_tsne[:, 1], c=y_all, s=8, alpha=0.8, cmap=cmap)

#     # 3. Добавляем центры категорий и подписи (логика как в PCA)
#     for i in range(len(target_names)):
#         points = X_tsne[y_all == i]
#         if len(points) > 0:
#             center = points.mean(axis=0)
#             ax.text(
#                 center[0],
...
#     ax.set_title(f"t-SNE Projection: {method_name}", fontsize=15)
#     ax.set_xlabel("t-SNE feature 1", fontsize=12)
#     ax.set_ylabel("t-SNE feature 2", fontsize=12)

#     # 4. Создаем легенду (полностью копируем твою логику с номерами)
#     legend_handles = []
#     for i, name in enumerate(target_names):
#         color = cmap(i)
#         label_text = f"{i}: {name}"
